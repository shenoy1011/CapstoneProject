...
model = YOLO("rootfs/models/best19.onnx")
...
bridge = CvBridge()
tracker = Tracker(distance_function="euclidean", distance_threshold=1000, detection_threshold=0.5, initialization_delay = 2)
...
def relevanceFilter(yolo_detections):
    ...
    max_area = max(areas)
    for b in detections_as_xyxy:
        if (b[0][2] - b[0][0]) * (b[0][3] - b[0][1]) >= (max_area * 0.4):
...
def callback_Img(data):
    ...
    res = model.predict(cv2_img[256:1024, 512:1536], show=False)
    ...
    for track in tracked_objects:
        ...
        if (ratio > 0.3 and ratio < 0.7):
            cv2.putText(cv2_img[256:1024, 512:1536], str(track.id) + ': blinking ' + classes[track.last_detection.label], (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
        else:
            cv2.putText(cv2_img[256:1024, 512:1536], str(track.id) + ': ' + classes[track.last_detection.label], (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
